...
\title{Quadratic Presentational Complexity and Exact Observer Barriers for Class-Two $p$-Groups}
\author{Luca Blanchi}
\date{June 2026}
...
Thus, from the perspective of $\mathcal N_{2,p}$, every word observer is a quadratic observer. We call the resulting theory quadratic presentational complexity. Its basic objects are quadratic presentations
...
Thus, on graphical groups, quadratic presentational observers are exactly right homomorphism counts toward alternating-orthogonality targets. This viewpoint gives:

\begin{itemize}
\item a canonical semantic normal form for quadratic observers on graphical groups;
\item a complexity dichotomy for evaluating a fixed observer;
\item a right-profile completeness theorem;
\item and an exact arity lower bound using Wilson's examples.
The final theorem says that there exist nonisomorphic groups in $\mathcal N_{2,p}$ that cannot be distinguished by any coefficient-free observer in fewer than
...
central lifts. This proves the formula. $\square$ Thus, on graphical (p)-groups, every quadratic observer is a right homomorphism count toward a finite alternating-orthogonality target.
...
Two quadratic presentations may have different variable and relation spaces but produce identical observers on all graphical groups. The weighted core gives the exact criterion.
...
\section{Coefficient-free observers and arity}

We now turn from graphical groups to arbitrary finite groups in $\mathcal N_{2,p}$. A coefficient-free observer in (k) variables is a Boolean combination of word equations and disequations in variables

\[
x_1,\dots,x_k
\]

with no named constants from the ambient group. For such an observer $\Phi$, write
...
to be the least (k) such that there exists a coefficient-free observer $\Phi$ in (k) variables with
...
Let $\Phi$ be a coefficient-free observer in (k) variables. For every finite group (G),
...
then every coefficient-free observer in (k) variables has the same number of solutions in (G) and (H).
...
\begin{theorem}[Exact observer arity barrier]
...
all groups in $\mathcal W_{p,e}$ have identical values for every coefficient-free observer in (k) variables.
...
for every $G\in\mathcal W_{p,e}$, no (k)-tuple generates (G). Hence every (k)-tuple generates a proper subgroup. All groups in $\mathcal W_{p,e}$ have the same multiset of isomorphism types of proper subgroups. By Corollary 16.2, every coefficient-free observer in (k) variables has the same number of solutions in all groups in the family. Thus
...
Theorem 18.2 shows that arity is a genuine resource for coefficient-free observers. It is not enough to use:

\begin{itemize}
\item more relators;
\item longer relators;
\item more complicated Boolean combinations;
if the number of variables is below the generator rank. For the Wilson families, every such observer is trapped inside proper subgroups, and the proper subgroup profiles are identical. Thus the subgroup-profile obstruction is not merely an obstruction to a particular algorithm or heuristic. It obstructs the entire coefficient-free observer hierarchy below the generator rank.
...
In $\mathcal N_{2,p}$, every word observer is quadratic.

\subsection{Right-profile semantics}

On graphical (p)-groups, every quadratic observer is exactly a right homomorphism count toward an alternating-orthogonality target.
...
Two observers have the same values on all graphical groups if and only if their weighted cores agree.

\subsection{Complexity dichotomy}

Every fixed nondegenerate observer gives a ($\#P$)-complete counting problem on input graphs.
...
variables to distinguish by any coefficient-free observer.
...
This paper studies counting observers, not canonical forms. A natural next goal is to turn right-profile or quadratic-presentational data into canonical decompositions for alternating maps
