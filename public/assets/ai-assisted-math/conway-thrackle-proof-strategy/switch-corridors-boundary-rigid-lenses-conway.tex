...
\hypersetup{
  ...
  citecolor=blue,
  urlcolor=blue,
  pdftitle={Switch Corridors and Boundary-Rigid Lenses in Even Dumbbell Thrackles: A Local Reduction and Minimal-Witness Strategy},
...
\title[Switch Corridors: Local Reduction]{Switch Corridors and Boundary-Rigid Lenses in Even Dumbbell Thrackles: A Local Reduction and Minimal-Witness Strategy}
\author{Luca Blanchi}
\date{June 2026}
...
\begin{abstract}
We give a detailed reduction framework for the irreducible even-dumbbell stage of Conway's thrackle program. The starting point is a non-\(T_3\) even dumbbell thrackle. A finite domain-incidence argument first selects two ordinary same-cycle vertices \(p,q\) with separated incident-domain data. The two complementary \(p\)-to-\(q\) paths are then treated as anchored traces rather than as free local corridors: every subsequent simplification must preserve the original endpoint data or record a precise lower subproblem. From this anchored setup we construct typed carriers, prove a no-backtracking theorem by a well-founded minimal-witness cost, and compile the remaining reachability data into ordered switch corridors. The corridor analysis separates regular switching cells from the genuinely singular residuals: common-vertex marker data, support-gate disks containing original vertices, blocked R3 moves, replacement lenses, and endpoint failures. Each residual is routed to a terminal contradiction, a strict descent, a lower named subproblem, or a finite local verification. The internal closure theorem states that, once the listed finite local verifications are supplied, no unresolved irreducible non-\(T_3\) even-dumbbell obstruction survives. Thus the manuscript reduces the internal Conway step to a finite collection of explicit local checks together with the standard external reductions to the even-dumbbell and \(T_3\) cases.
\end{abstract}
...
We use the word \textbf{certificate} only in the standard finite-witness sense: a
...
\section{Witness Packages and the Global Cost}
...
\subsection{Finite descriptions of Conway obstruction packages}
...
A description is \textbf{unresolved} if it realizes a nonterminal obstruction package
...
\end{lemma}
\begin{proof}
The lexicographic order on \(\mathbb N^{12}\) is well-founded. The
...
\subsection{Cost-controlled local outputs}
...
\begin{theorem}[No cycle of local reductions]
Assume each invoked local block
...
\item \(C\) or \(R\): a verified R3 or verified replacement obligation for B7;
...
\end{proof}
\begin{lemma}[Strong replacement data]
Let \(R=(\partial R,\alpha_R,\beta_R,\tau_R,\omega_R)\) be a pending replacement obligation. A strong replacement datum consists of the following finite data:
...
\end{proof}
\begin{lemma}[Local replacement-failure reduction]
A local failure of strong
...
\end{proof}
\begin{lemma}[Domain-data persistence]
Let \(T\leadsto T'\) be a graph-changing
admissible even-dumbbell surgery with strong local data. Suppose B1 domain data
for \(T\notin T_3\) survive outside the surgery lens:

\begin{enumerate}
\item all ordinary matched vertices survive as ordinary vertices of \(T'\);
\item the replacement lens is disjoint from their small neighborhoods and from the common vertex;
\item the outside face bijection preserves the two incident domains of every matched vertex;
...
B7 closes only after its finite local data have
...
\begin{definition}[Admissible Conway reduction catalogue]
A Conway reduction
...
\begin{enumerate}
\item Write the finite-verification appendices. The manuscript still needs the full
...
\item Preserve the global anchors and records. No local transfer may silently
