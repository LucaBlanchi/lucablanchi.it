...
\begin{document}
\title[Switch Corridors in Even Dumbbell Thrackles]{Switch Corridors and Boundary-Rigid Lenses in Even Dumbbell Thrackles: A Proof Strategy for Conway's Conjecture}
\author{Luca Blanchi}
\date{June 2026}
\begin{abstract}
...
&\Rightarrow \text{even dumbbell}\\
&\Rightarrow \text{irreducible even dumbbell}\\
&\Rightarrow T_3\\
&\Rightarrow |E|\le |V|,
\end{aligned}
\]
...
This note is shared as a preliminary research note. Some arguments, reductions, or technical lemmas may still contain gaps, errors, or unverified steps; the text should be read as a working draft rather than as a fully checked manuscript.

\section{External Inputs, Conventions, and Standing Definitions}
...
\begin{enumerate}
\item if two edges are adjacent, their unique meeting point is their common endpoint;
\item if two edges are non-adjacent, they meet in exactly one proper crossing.
\end{enumerate}
...
\subsection{External Input I: reduction to even dumbbells}
...
\subsubsection*{Theorem 1.1 --- Even dumbbell reduction}
...
\subsection{External Input II: the $T_3$-thrackle theorem}
...
\subsubsection*{Theorem 1.2 --- $T_3$-thrackles satisfy Conway}
...
\subsection{External Input III: edge removal and irreducibility}
...
\subsubsection*{Theorem 1.3 --- Edge-removal criterion}
...
\subsection{Summary of the external framework}
...
\section{Reduction of Conway's Conjecture to the Internal Theorem}

\subsection{Statement of Conway's conjecture}

\subsubsection*{Theorem 2.1 --- Conway's thrackle conjecture}
...
\subsection{The internal theorem used in the reduction}

The internal theorem is the following.

\subsubsection*{Theorem 2.2 --- Internal even-dumbbell theorem}
...
\subsection{Proof that the internal theorem implies Conway}
...
\section{The Internal Even-Dumbbell Theorem}
...
\subsubsection*{Theorem 3.1 --- Irreducible even dumbbell thrackles are $T_3$}
...
\subsection{Step II: switch corridors}
...
\subsection{Step III: regularization of switch corridors}

The central technical theorem is the switch-corridor regularization theorem.

\subsubsection*{Theorem 3.3 --- Switch-corridor regularization}
...
\begin{enumerate}
\item an edge-removal;
\item a double crossing;
\item a repeated support;
\item a pure ear;
\item a relevant inner domain or smaller $T_3$-critical core;
\item a smaller non-identity relational subcorridor;
\item a gate identification;
\item a terminal lane;
\item a returning lane;
\item a branching or merging lane;
\item a closed alternating train.
\end{enumerate}
...
\subsection{Step IV: regular corridors produce ladders or trains}
...
\begin{itemize}
\item vertices equal to gates;
\item edges equal to lane continuations.
\end{itemize}
...
\subsubsection*{Lemma 3.4 --- Gate graph dichotomy}

A regular switch corridor contains either a local ladder or a closed alternating train.

\subsection{Step V: complementary ladders close into trains}
...
\subsubsection*{Lemma 3.5 --- Complementary ladder closure}
...
\subsection{Step VI: closed alternating trains are impossible}

The final structural theorem is the train-impossibility theorem.

\subsubsection*{Theorem 3.6 --- No closed alternating trains}
...
\subsection{Completion of the internal theorem}
...
\begin{enumerate}
\item Geometric realization of switch corridors as relational corridors.
\item Boundary rigidity of reduced quadrilaterals.
\item Switch-corridor regularization.
\item Gate graph dichotomy:
\end{enumerate}
...
\begin{enumerate}
\item Complementary ladder closure.
\item Impossibility of closed alternating trains.
\end{enumerate}

These are proved in the subsequent sections.

\section{Planarization, Domains, and the $T_3$ Condition}
...
\begin{itemize}
\item $V_{\mathrm{orig}}$ is the set of original vertices of $F_{2a,2b}$;
\item $V_\times$ is the set of crossing vertices introduced by planarization.
\end{itemize}
...
\subsection{Ordinary vertices and their two incident domains}
...
\subsubsection*{Lemma 4.1 --- Ordinary vertices have two incident domains}
...
\begin{itemize}
\item every crossing vertex has degree (4);
\item every ordinary original vertex has degree (2);
\item the common vertex $v$ has degree (4).
\end{itemize}
...
\subsection{The $T_3$-condition as a hitting problem}
...
\subsection{Eulerian planar graphs and bipartite duals}
...
\subsubsection*{Lemma 4.2 --- Eulerian plane graphs have bipartite face-duals}
...
\section{The Domain Graph and the Domain-Pair Separation Lemma}
...
\subsection{The ordinary domain graph}
...
\subsection{Covering ordinary vertices as a vertex cover problem}
...
\subsubsection*{Theorem 5.3 --- König}
...
\subsection{The domain-pair separation lemma}

We now prove the main result of this section.

\subsubsection*{Lemma 5.4 --- Domain-pair separation}
...
\textbackslash\{\}textbf\{Case 1: the ordinary vertices are not coverable by three domains\}
...
\textbackslash\{\}textbf\{Case 2: the ordinary vertices are coverable by three domains\}
...
\begin{itemize}
\item every edge of $G_D$ incident to $d$ is covered by $d$;
\item every edge not incident to $d$ lies in $G_D-d$ and is covered by $S$.
\end{itemize}
...
\section{Switch Traces and Switch Corridors}
...
\begin{enumerate}
\item the parameter path $\gamma$ in the abstract graph;
\item its drawn image in the plane;
\item the corresponding walk in the planarized graph (P(T)).
\end{enumerate}
...
\subsection{Edgelets along $\gamma$}
...
\begin{enumerate}
\item a crossing vertex where the traversed original edge crosses another original edge;
\item an original vertex of the cycle $A$;
\item a repeated crossing vertex of the drawn path, visited at different parameter times.
\end{enumerate}
...
\subsection{Raw switch trace}

The raw switch trace associated with $(p,q,\gamma)$ is the finite combinatorial object consisting of:

\begin{enumerate}
\item the parameterized planarized walk determined by $\gamma$;
\item the sequence of lateral domain pairs
\end{enumerate}
...
\begin{enumerate}
\item the planarized vertices at which the transitions
\end{enumerate}
...
\begin{enumerate}
\item small transverse endpoint sections near $p$ and $q$, meeting the two lateral domains of (I(p)) and (I(q)), respectively.
\end{enumerate}
...
\subsection{Geometric corridors}
...
\begin{enumerate}
\item $\Sigma_-$ lies near the initial side of a switch subtrace;
\item $\Sigma_+$ lies near the terminal side of that subtrace;
\item the intersection $P(T)\cap C$ is finite and transverse to the sections;
...
\begin{enumerate}
\item this boundary relation is part of the transformation of lateral domain data from (I(p)) to (I(q)).
\end{enumerate}
...
\subsection{Boundary states of a corridor}
...
\begin{itemize}
\item (f(C)) is the number of relative domains of $C\setminus P(T)$;
\item (c(C)) is the number of crossing vertices of (P(T)) lying in the interior of $C$;
\item (e(C)) is the number of edgelets of $P(T)\cap C$;
\item (b(C)) is the combinatorial length of $\partial C\cap P(T)$;
\item (s(C)) is the total number of boundary states on $\Sigma_-\cup\Sigma_+$.
\end{itemize}
...
\subsection{Relational corridors}
...
\begin{itemize}
\item $\Sigma_-$ and $\Sigma_+$ are transverse sections;
\item $U,V$ are longitudinal boundary arcs;
\item (P(T)) meets the transverse sections transversely;
\item no vertex of (P(T)) lies in the interior of either transverse section.
\end{itemize}
...
\subsection{Reducibility and regularity preview}
...
\item pure ear;
\item relevant inner domain;
\item gate identification;
\item terminal lane;
\item returning lane;
\item branching or merging lane;
\item closed alternating train.
\end{itemize}
...
\subsection{Summary of the switch-corridor setup}
...
\subsection{Normal quadrilaterals}
...
\begin{enumerate}
\item $\Sigma_-$ and $\Sigma_+$ are closed PL intervals;
\item $U$ and $V$ are closed PL arcs, called the longitudinal sides;
\item the interiors of the four boundary pieces are pairwise disjoint;
\item $\Sigma_-\cap \Sigma_+$ is empty;
\item $\Sigma_-$ and $\Sigma_+$ meet $\Gamma$ transversely;
\item no vertex of $\Gamma$ lies in the interior of $\Sigma_-\cup\Sigma_+$;
\item $\Gamma\cap |Q|$ is finite.
\end{enumerate}

We usually suppress the vertical bars and write simply $Q$ for the underlying disk.

The transverse intervals $\Sigma_-$ and $\Sigma_+$ are called the input section and output section. The labels `\textbackslash\{\}texttt\{input'' and \}`output'' do not carry orientation data; they only indicate the direction in which relations will later be composed.
...
\subsection{Lanes in a normal quadrilateral}
...
\begin{itemize}
\item passing if its closure meets both $\Sigma_-$ and $\Sigma_+$;
\item terminal if its closure meets exactly one of the two sections;
\item returning if its closure meets the same section in two or more distinct states.
\end{itemize}
...
\subsection{Boundary relation of a quadrilateral}
...
\subsubsection*{Lemma 7.1 --- Path characterization of the boundary relation}
...
\subsection{Ordered states and identity relations}
...
\subsubsection*{Lemma 7.2 --- Identity subcorridors are removable}
...
\subsection{Minimal relational corridors}
...
\begin{itemize}
\item $N$ is the number of quadrilaterals in $\mathcal C$;
\item $F$ is the total number of relative domains in the quadrilaterals;
\item $C$ is the total number of crossing vertices contained in the quadrilaterals;
\item $E$ is the total number of edgelets of $\Gamma$ contained in the quadrilaterals;
\item $B$ is the total combinatorial length of the longitudinal boundaries;
\item $S$ is the total number of states on all internal transverse sections.
\end{itemize}
...
\subsection{Minimal corridors contain no identity subcorridors}
...
\section{Geometric Realization of Switch Corridors}
...
\subsection{Geometric two-terminal corridors}
...
\begin{enumerate}
\item $C\subset\mathbb R^2$ is a closed PL disk;
\item $\Sigma_-,\Sigma_+\subset\partial C$ are disjoint closed PL intervals;
\item $\Sigma_-$ and $\Sigma_+$ meet $\Gamma$ transversely;
\item no vertex of $\Gamma$ lies in the interior of $\Sigma_-\cup\Sigma_+$;
\item the remaining two components of
\end{enumerate}
...
\subsection{Generic PL height functions}

The main tool is slicing the corridor by a generic height function.

\subsubsection*{Lemma 8.1 --- Existence of a generic height}
...
\begin{enumerate}
\item no edgelet of $\Gamma\cap C$ is contained in a level set of $h$;
\item each vertex of $\Gamma\cap\operatorname{int}C$ has a distinct $h$-value;
\item no vertex of $\Gamma$ lies on the transverse levels used for slicing;
\item all intersections of $\Gamma$ with regular levels of $h$ are transverse.
\end{enumerate}
...
\subsection{Slicing into normal quadrilaterals}
...
\begin{enumerate}
\item each $t_i$ is a regular value of $h|_{\Gamma\cap C}$;
\item every interior vertex of $\Gamma\cap C$ has height lying in exactly one interval
\end{enumerate}
...
\begin{enumerate}
\item each interval $(t_{i-1},t_i)$ contains at most one vertex-height.
\end{enumerate}
...
\subsection{Equality between geometric and relational boundary relations}

\subsubsection*{Proposition 8.2}
...
\subsection{Subdisks and subcorridors}
...
\subsubsection*{Lemma 8.3 --- Internal subdisks are contained in proper subcorridors}
...
\subsection{Identity subcorridors and deletion}

\subsubsection*{Proposition 8.4}
...
\subsection{Non-identity subcorridors}

\subsubsection*{Proposition 8.5}
...
\subsection{Geometric Realization Lemma}

We now collect the preceding results.

\subsubsection*{Theorem 8.6 --- Geometric Realization Lemma}
...
\begin{enumerate}
\item each $Q_i$ is a normal quadrilateral;
\item the geometric boundary relation equals the composed relational boundary relation:
\end{enumerate}
...
\begin{enumerate}
\item every compact internal subdisk is contained, after refinement if necessary, in a proper relational subcorridor;
\item an identity subcorridor may be removed from the witness without changing the total boundary relation;
\item a non-identity subcorridor is a proper smaller relational corridor.
\end{enumerate}
...
\section{Reduced Quadrilaterals and Boundary Rigidity}
...
\subsection{Lanes and degeneracies}
...
\subsection{Reduced normal quadrilaterals}

A normal quadrilateral $Q$ is called reduced if none of the following configurations occurs inside $Q$:

\begin{enumerate}
\item an edge-removal triangle empty of original vertices;
\item a double crossing of two original supports;
\item an essential repeated support;
\item a pure ear;
\item a relevant inner domain or smaller $T_3$-critical core;
\item a proper non-identity relational subcorridor;
\item a gate identification;
\item a terminal lane;
\item a returning lane;
\item branching or merging of lanes;
\item a closed alternating train.
\end{enumerate}
...
\subsection{Every boundary state belongs to a passing lane}

\subsubsection*{Lemma 9.1}
...
\subsection{A passing lane meets each section in exactly one state}

\subsubsection*{Lemma 9.2}
...
\subsection{The boundary relation is a bijection}

\subsubsection*{Lemma 9.3}
...
\subsection{Core arcs inside lanes}

\subsubsection*{Lemma 9.4}
...
\subsection{Noncrossing arcs preserve order}

\subsubsection*{Lemma 9.5}
...
\subsection{Boundary-rigidity theorem}

\subsubsection*{Theorem 9.6 --- Boundary rigidity of reduced quadrilaterals}
...
\subsection{Consequence for reduced lenses}
...
\subsection{Colored support arcs}

Let $Q$ be a disk-like chamber inside a switch corridor. Its boundary is a finite concatenation of two types of arcs:

\begin{enumerate}
\item support arcs, which are subarcs of original edge supports of the drawing;
\item gate arcs, which are transverse intervals through which lanes enter or leave the chamber.
\end{enumerate}
...
\subsection{Alternating and non-alternating chambers}
...
\subsection{Monochromatic lenses}
...
\begin{enumerate}
\item the monochromatic side $U_\rho$;
\item the complementary side $V_\rho$.
\end{enumerate}
...
\subsection{Reduced monochromatic lenses}
...
\item repeated support;
\item pure ear;
\item relevant inner domain or smaller $T_3$-critical core;
\item proper non-identity relational subcorridor;
\item gate identification;
\item terminal lane;
\item returning lane;
\item branching or merging lane;
...
\subsubsection*{Lemma 10.2 --- Reduced monochromatic lenses are identity}
...
\subsection{Monochromatic lenses in minimal corridors}

We now prove the key lemma of this section.

\subsubsection*{Lemma 10.3 --- Monochromatic lenses are reducible in minimal corridors}
...
\subsection{Consequence for non-alternating chambers}

\subsubsection*{Corollary 10.4}
...
\section{Regularization of Switch Corridors}
...
\begin{enumerate}
\item edge-removal: an empty edge-removal triangle for a 3-path;
\item double crossing: two original supports meet more than once;
\item repeated support: an original support appears twice in an essential boundary or lane itinerary;
\item pure ear: a disk bounded by portions of the corridor and the planarized graph, containing no original vertices and no essential boundary relation;
\item relevant inner domain: an internal disk containing original vertices and producing either a smaller $T_3$-critical core or a smaller relational corridor;
\item proper non-identity subcorridor: a proper subcorridor with non-identity boundary relation;
\item gate identification: two distinct gates represent the same topological boundary interval;
\item terminal lane: a lane meets exactly one transverse section and is not eliminable as a pure ear;
\item returning lane: a lane meets the same transverse section in more than one state;
\item branching or merging lane: the continuation of lanes is not locally one-to-one;
\item closed alternating train: a cyclic component of the gate graph;
\item proper subtrain: a smaller closed train contained in a train.
\end{enumerate}
...
\subsection{Compensation chambers}
...
\begin{enumerate}
\item its boundary is a finite cyclic concatenation of support arcs and gate arcs;
\item each support arc has color $A$ or $B$;
\item it induces a local boundary relation on the gate states incident to it;
\item it is minimal with respect to inclusion among chambers realizing that local compensation.
\end{enumerate}

A chamber is embedded if its boundary is a simple closed curve in the planarized disk.

\subsection{Closed-walk innermost lemma}

We shall repeatedly use the following planar fact.

\subsubsection*{Lemma 11.1 --- Innermost closed-walk reduction}

Let $W$ be a closed walk in the planarized graph inside a switch corridor. Then one of the following occurs:

\begin{enumerate}
\item $W$ contains an embedded simple cycle $Z$;
\item $W$ contains a repeated support;
\item $W$ contains a double crossing;
\item $W$ contains a gate identification.
\end{enumerate}
...
\begin{enumerate}
\item a pure ear;
\item a relevant inner domain;
\item a proper identity subcorridor;
\item a proper non-identity subcorridor;
\item a closed alternating train or proper subtrain.
\end{enumerate}
...
\begin{itemize}
\item repetition of an original support;
\item repeated intersection of two original supports, i.e. double crossing;
\item identification of gate intervals.
\end{itemize}
...
\subsection{Short alternating chambers}
...
\subsubsection*{Lemma 11.2 --- Short alternating chambers are reducible}
...
\subsubsection*{Lemma 11.3 --- Gate continuation is unique in a non-reducible corridor}
...
\subsubsection*{Lemma 11.4 --- Alternating cells meet only along gates}
...
\subsection{Definition of regular corridor}

A switch corridor is regular if it satisfies all of the following:

\begin{enumerate}
\item every compensation chamber is an alternating cell $H_{2m}$ with $m\ge3$;
\item distinct cells meet only along gates;
\item no three cells share a gate;
\item every internal gate has unique continuation;
\item the only terminal gates are on the two endpoint sections of the corridor.
\end{enumerate}

\subsection{Switch-corridor regularization theorem}

\subsubsection*{Theorem 11.5 --- Regularization}
...
\subsection{Closed alternating trains}
...
\subsection{Same-side path components are reducible}

\subsubsection*{Lemma 12.1}
...
\item a proper identity subcorridor;
\item a proper non-identity subcorridor;
\item a closed train or subtrain.
\end{enumerate}
...
\subsection{Gate graph dichotomy}

\subsubsection*{Proposition 12.2 --- Ladder/train dichotomy}

Let $\mathcal C$ be a minimal non-reducible regular corridor. Then $\mathcal C$ contains either:

\begin{enumerate}
\item a local ladder; or
\item a closed alternating train.
\end{enumerate}
...
\subsection{Interpretation for switch corridors}
...
\section{Complementary Ladders Close into Trains}
...
\subsection{Local structure near an ordinary vertex}

We first record the local topology near an ordinary vertex.

\subsubsection*{Lemma 13.1 --- Two-domain local pairing}
...
\begin{itemize}
\item a lane has no continuation, giving a terminal lane;
\item two different gates represent the same local sector, giving a gate identification;
\item one lane splits into two or two lanes merge into one, giving branching or merging;
\item a closed disk is cut off near $x$ with no original vertices inside, giving a pure ear;
\item a closed disk is cut off containing original vertices, giving a relevant inner domain.
\end{itemize}
...
\subsection{Endpoint gates of a local ladder}
...
\begin{itemize}
\item the input terminal gates lie on a transverse section near $p$;
\item the output terminal gates lie on a transverse section near $q$.
\end{itemize}
...
\subsection{Complementary ladder closure}

\subsubsection*{Proposition 13.2 --- Complementary ladders close}
...
\begin{enumerate}
\item one of them already contains a closed alternating train;
\item both contain local ladders, and the ladders close into a closed alternating train.
\end{enumerate}

Thus:

\subsubsection*{Corollary 13.3}
...
\section{Closed Alternating Trains Are Impossible}
...
\begin{enumerate}
\item every cell is an alternating cell $H_{2m}$ with $m\ge3$;
\item every internal gate has unique continuation;
\item there are no terminal lanes;
\item there is no branching or merging;
\item there are no gate identifications;
\item no two original supports cross twice inside the train;
\item no support is repeated in a way already classified as reducible.
\end{enumerate}
...
\subsection{Orientation of cells}
...
\subsection{Opposite orientations are reducible}

\subsubsection*{Lemma 14.1 --- No adjacent inverse orientations in a minimal train}
...
\subsection{Uniform orientation}

\subsubsection*{Corollary 14.2}
...
\subsection{Fixed-support blocks and positive braids}
...
\subsubsection*{Lemma 14.3 --- Positive pure braid gives double crossing}
...
\subsection{Consequence for fixed-support trains}

\subsubsection*{Lemma 14.4 --- Fixed-support train segments are impossible}
...
\subsection{First repeated support}

\subsubsection*{Lemma 14.5 --- A proper repeated support is reducible}
...
\subsection{Support-simple itineraries}

\subsubsection*{Lemma 14.6 --- Support-simple closed lanes are reducible}
...
\begin{enumerate}
\item the same support is encountered twice, contradicting support-simplicity;
\item two distinct supports meet twice, giving a double crossing;
\item two gate intervals are identified;
\item a lane branches or merges.
\end{enumerate}
...
\subsection{Impossibility of closed trains}

\subsubsection*{Theorem 14.7 --- No closed alternating trains}
...
\section{Proof of the Internal Even-Dumbbell Theorem}

We now assemble the internal proof.

Recall the statement.

\subsubsection*{Theorem 15.1 --- Irreducible even dumbbell thrackles are $T_3$}
...
\subsection{Applying regularization}
...
\item pure ear;
\item relevant inner domain or smaller $T_3$-critical core;
\item proper non-identity subcorridor;
\item gate identification;
\item terminal lane;
\item returning lane;
\item branching or merging lane;
...
\begin{itemize}
\item Edge-removal contradicts irreducibility of $T$.
\item Double crossing contradicts the definition of a thrackle drawing.
\item Repeated support, gate identification, returning lane, terminal lane, and branching or merging produce, by the innermost closed-walk reduction, either a pure ear, a relevant inner domain, a smaller corridor, a double crossing, or a closed train.
\item A pure ear is eliminable from the witness corridor and contradicts minimality.
\item A relevant inner domain yields either a smaller $T_3$-critical core or a smaller nontrivial relational corridor, contradicting minimality of the chosen obstruction.
\item A proper non-identity subcorridor contradicts minimality of the chosen corridor.
\item A closed alternating train contradicts Theorem 14.7.
\end{itemize}
...
\subsection{Regular corridors yield ladders or trains}
...
\subsection{Complementary ladders close}
...
\subsection{Consequence for even dumbbells}
...
\section{Completion of the Proof of Conway's Thrackle Conjecture}
...
\begin{enumerate}
\item it is enough to exclude thrackle drawings of even dumbbells (DB(c',c'',0));
\item every $T_3$-thrackle satisfies Conway's inequality.
\end{enumerate}
...
\subsection{Minimal counterexamples and irreducibility}
...
\subsection{Applying the internal theorem}
...
\section{Compatibility of Definitions and External Inputs}
...
\subsection{Compatibility with the even-dumbbell notation}
...
\subsection{Counting vertices and edges}
...
\subsection{Compatibility of the $T_3$ convention}
...
\subsection{Compatibility of irreducibility and minimality}
...
\subsection{Compatibility of planarization}
...
\begin{enumerate}
\item the domains of $T$ and (P(T)) are the same connected regions of the plane;
\item the boundary-incidence relation
\end{enumerate}
...
\begin{enumerate}
\item the $T_3$-condition may be checked on the planarized graph.
\end{enumerate}

This justifies using (P(T)) throughout the proof while invoking a theorem stated for the original drawing.

\section{Dependency Summary and End-of-Proof Checklist}
...
\subsection{Internal proof dependency chain}
...
\item planarization;
\item domains;
\item the ordinary domain graph $G_D$;
\item bipartiteness of the face-dual;
\item König's theorem.
\end{itemize}
...
\begin{itemize}
\item geometric switch traces;
\item PL slicing;
\item relational corridors;
\item the Geometric Realization Lemma.
\end{itemize}
...
\begin{itemize}
\item reduced quadrilaterals;
\item boundary rigidity;
\item monochromatic lens elimination;
\item innermost closed-walk reductions;
\item short-cell elimination.
\end{itemize}
...
\begin{itemize}
\item uniform-orientation reduction;
\item positive braid/full-twist obstruction;
\item support-shift analysis;
\item first repeated support and innermost disk arguments.
\end{itemize}

Therefore $T\notin T_3$ is impossible.

\subsection{Reducible configurations and their contradictions}
...
\subsection{Key local topological facts}
...
\bibitem[Woo71]{Woo71} D. R. Woodall, Thrackles and deadlock, in D. J. A. Welsh, ed., Combinatorial Mathematics and its Applications: Proceedings of the Conference held at the Mathematical Institute, Oxford, 1969, Academic Press, London, 1971, pp. 335--347.
\bibitem[CK99]{CK99} G. Cairns and D. M. King, The answer to Woodall's musquash problem, Discrete Mathematics 207(1--3) (1999), 25--32.
\bibitem[CK01]{CK01} G. Cairns and D. M. King, All odd musquashes are standard, Discrete Mathematics 226(1--3) (2001), 71--91.
\bibitem[LPS97]{LPS97} L. Lovász, J. Pach, and M. Szegedy, On Conway's thrackle conjecture, Discrete \& Computational Geometry 18(4) (1997), 369--376.
\bibitem[CN00]{CN00} G. Cairns and Y. Nikolayevsky, Bounds for generalized thrackles, Discrete \& Computational Geometry 23(2) (2000), 191--206.
\bibitem[CMN04]{CMN04} G. Cairns, M. McIntyre, and Y. Nikolayevsky, The thrackle conjecture for $K_5$ and $K_{3,3}$, in Towards a Theory of Geometric Graphs, Contemporary Mathematics 342, American Mathematical Society, Providence, RI, 2004, pp. 35--54.
\bibitem[CN09]{CN09} G. Cairns and Y. Nikolayevsky, Generalized thrackle drawings of non-bipartite graphs, Discrete \& Computational Geometry 41(1) (2009), 119--134.
\bibitem[PS11]{PS11} J. Pach and E. Sterling, Conway's conjecture for monotone thrackles, The American Mathematical Monthly 118(6) (2011), 544--548.
\bibitem[CN12]{CN12} G. Cairns and Y. Nikolayevsky, Outerplanar thrackles, Graphs and Combinatorics 28(1) (2012), 85--96.
\bibitem[CKN15]{CKN15} G. Cairns, T. J. Koussas, and Y. Nikolayevsky, Great-circle spherical thrackles, Discrete Mathematics 338(12) (2015), 2507--2513.
\bibitem[FP11]{FP11} R. Fulek and J. Pach, A computational approach to Conway's thrackle conjecture, Computational Geometry: Theory and Applications 44(6--7) (2011), 345--355.
\bibitem[GX17]{GX17} L. Goddyn and Y. Xu, On the bounds of Conway's thrackles, Discrete \& Computational Geometry 58(2) (2017), 410--416.
\bibitem[FP19]{FP19} R. Fulek and J. Pach, Thrackles: an improved upper bound, Discrete Applied Mathematics 259 (2019), 226--231.
\bibitem[Xu21]{Xu21} Y. Xu, A new upper bound for Conway's thrackles, Applied Mathematics and Computation 389 (2021), Article 125573.
\bibitem[MN18]{MN18} G. Misereh and Y. Nikolayevsky, Annular and pants thrackles, Discrete Mathematics \& Theoretical Computer Science 20(1) (2018), Article \#16.
\bibitem[Mis18]{Mis18} G. Misereh, Thrackles containing a standard musquash, Australasian Journal of Combinatorics 70(2) (2018), 168--182.
\bibitem[HVKS25]{HVKS25} C. Hernández-Vélez, J. Kyn\v{c}l, and G. Salazar, Thrackles on nonplanar surfaces, arXiv:2506.11808, 2025.
\end{thebibliography}
\end{document}
