...
\textbf{Target reading.}
Even singular Hensel trees have finite-memory statistics. The number of dead nodes, nodes with a fixed branching number, or nodes with a prescribed survival profile cannot vary chaotically with \(k\); it is governed by a rational generating function.
...
\begin{remark}
This example is a family of transfers, not many unrelated examples. The single source engine is \(p\)-adic definability plus rationality of counting series; the target observables are different finite views of the same lifting tree.
\end{remark}
