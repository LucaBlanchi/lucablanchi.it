...
\omega(x)\notin S_b,
\]
then \(x\notin\mathcal X_\Gamma^{\leq b}\). In particular, when \(C_\Gamma\) is defined, \(C_\Gamma(x)>b\).
\end{proposition}
...
\]
The verification cost of \(x\in P\) is
\[
...
\]
whenever the displayed quantities are defined and \(f\) is monotone.
\end{theorem}
...
For the complexity inequality, choose descriptions of \(x\) with cost arbitrarily close to \(C_{\Gamma_\mathcal X}(x)\), apply the first part, and take the infimum. In well-ordered scales this is simply the same argument with a minimum-cost description.
\end{proof}
...
\]
A relative presentation theory for the fibres of \(q\) assigns to each \(y\in\mathcal Y\) a class of relative descriptions of objects in \(q^{-1}(y)\), with a relative cost. The relative complexity is denoted
\[
...
The map \(q\) has controlled assembly with overhead \(A\) if a description of \(q(x)\), together with a relative description of \(x\) over \(q(x)\), can be assembled into a description of \(x\) of cost at most
\[
...
If \(q\) has controlled assembly, then
\[
C_\mathcal X(x)
\leq
A(C_\mathcal Y(q(x)),C_{\mathrm{rel}}(x\mid q(x))).
...
\begin{proof}
Choose a description of \(q(x)\) and a relative description of \(x\) over \(q(x)\). Applying the assembly procedure gives a description of \(x\) with the displayed cost. Taking infima over the two input descriptions gives the result.
\end{proof}

\begin{definition}[Controlled extraction]
The map \(q\) has controlled extraction if every description of \(x\) yields:
\begin{enumerate}[label=(\roman*)]
\item a description of \(q(x)\) with overhead \(a\);
\item a relative description of \(x\mid q(x)\) with overhead \(r\).
\end{enumerate}
\end{definition}

\begin{theorem}[Lower base-fibre bounds]
If \(q\) has controlled extraction, then
\[
...
Consequently,
\[
C_\mathcal Y(q(x))>a(b)\Longrightarrow C_\mathcal X(x)>b,
\]
and
\[
C_{\mathrm{rel}}(x\mid q(x))>r(b)\Longrightarrow C_\mathcal X(x)>b.
\]
\end{theorem}

\begin{proof}
Apply extraction to a description of \(x\) of cost at most \(b\). If \(C_\mathcal X(x)\leq b\), then the extracted base and relative descriptions have costs at most \(a(b)\) and \(r(b)\). The displayed implications are contrapositives.
\end{proof}

\begin{corollary}[Coarse base-fibre decomposition]
If both assembly and extraction are controlled in an overhead class \(\mathcal H\), then \(C_\mathcal X(x)\) is equivalent, up to \(\mathcal H\)-distortion, to the joint data of \(C_\mathcal Y(q(x))\) and \(C_{\mathrm{rel}}(x\mid q(x))\).
\end{corollary}
...
If a verification pullback exists with overhead \(h\), then
\[
v_{P_\mathcal X}(x)
\leq
...
\begin{proof}
Pull back target verification data for \(F(x)\) and use the overhead bound. Taking the infimum over target data gives the inequality.
\end{proof}

\begin{theorem}[Lower transfer for verification cost]
If a verification pushforward exists with overhead \(p\), then
\[
v_{P_\mathcal Y}(F(x))
\leq
p(v_{P_\mathcal X}(x)).
...
\begin{proof}
Push forward source verification data and take infima. The lower-bound statement is the contrapositive.
\end{proof}
...
\begin{theorem}[Controlled transfer metatheorem]
Let
\[
\mathfrak T:\mathfrak P_\mathcal X\to\mathfrak P_\mathcal Y
\]
...
\[
0<\varepsilon<\frac{\beta-\alpha}{3}.
\]
Given \(e\), compute
\[
N=M(e,\varepsilon).
\]
Compute \(a_{e,N}\) to error less than \(\varepsilon\). If the computed approximation is above \((\alpha+\beta)/2\), answer \(a_e\geq\beta\); if it is below \((\alpha+\beta)/2\), answer \(a_e\leq\alpha\). The two promised cases are separated by more than \(2\varepsilon\), so the answer is correct. This decides the promised threshold problem, contradiction.
\end{proof}
...
D(G,\varepsilon)
\]
such that strategies of local Hilbert-space dimension at most \(D(G,\varepsilon)\) approximate the entangled value \(\omega_q(G)\) to within \(\varepsilon\) for every game \(G\) in the family.
\end{theorem}
...
If a computable dimension budget \(D(G,\varepsilon)\) existed, then \(\omega_q(G)\) would be computably approximable to arbitrary prescribed precision: compute \(d=D(G,\varepsilon)\) and approximate the finite-dimensional value \(\omega_{\leq d}(G)\). In particular, every constant-gap promise problem
\[
\omega_q(G)\leq\alpha
\qquad\text{or}\qquad
\omega_q(G)\geq\beta
\]
would be decidable. This contradicts the undecidability consequence of \( \mathrm{MIP}^{\ast}=\mathrm{RE}\), which gives finite nonlocal games with undecidable constant-gap entangled-value thresholds.
\end{proof}
...
\begin{proof}
Apply the controlled transfer metatheorem to the package \(\mathfrak P\to\mathfrak Q\) and then to the package \(\mathfrak Q\to\mathfrak P\). The comparison data for the composites identify the transported statements with the original ones, while the closure of \(\mathcal H\) under composition keeps the resulting overheads inside \(\mathcal H\). The listed profiles are defined by bounded existential or universal statements over the layers explicitly preserved by the equivalence, so their upper and lower bounds transport in both directions.
\end{proof}
