...
\title{Pfaffian Degree as Observer Complexity in Class-Two $p$-Groups}
\author{Luca Blanchi}
\date{June 2026}
...
\begin{abstract}
We prove exact observer-depth theorems for regular alternating pencils and the associated class-two exponent-$p$ groups. The results identify Pfaffian degree as the intrinsic obstruction to bounded-arity local observation. Let $p$ be odd. A regular affine alternating pencil is modeled by a finite $\mathbb F_p[t]$-module
...
The first main theorem is an exact support-image tomography theorem. For every structured CSI observer of arity (r), the rank transform is governed by Smith normal form:
...
Thus support-image observers of arity (r) see exactly the elementary-divisor data testable by polynomials of degree at most (r), and nothing more. The second main theorem extends the spectral barrier to arbitrary coefficient-free systems of word equations in the variety of class-two exponent-$p$ groups. After class-two normal form, every word system reduces to a finite system of quadratic commutator equations. A Fourier expansion and the skew-Smith normal form of linear alternating polynomial matrices show that every word observer in (k) variables factors through
...
Equivalently, if $f,h\in\mathbb F_p[t]$ are irreducible of degree (m) and are not projectively equivalent, then all coefficient-free word observers in fewer than (2m) variables have identical counts on the associated groups $G_f,G_h$, while two explicit word equations in (2m) variables distinguish them. Moreover, no single word equation can distinguish such $G_f,G_h$, regardless of the number of variables. The associated Baer groups $G_f$ are directly indecomposable special class-two exponent-$p$ groups of order
...
\[
\boxed{
\text{word observers in }k\text{ variables}
\Longleftrightarrow
...
detects (f) exactly at arity (m), whereas word observers require (2m) variables. This is optimal. A companion linearization
...
gives two explicit word equations in (2m) variables distinguishing the associated groups. No single word equation ever distinguishes irreducible blocks of the same degree. The conclusion is that Pfaffian degree is observer complexity.
...
\section{Word observers and QCO}
...
A finite Boolean combination of word equations and disequations is a finite integer linear combination of counts of systems of equations, by inclusion--exclusion. Thus the same analysis applies to quantifier-free Boolean word observers.
...
\begin{theorem}[Word observers factor through truncated spectral data]
...
Then every coefficient-free word observer in at most (k) variables has the same count on
...
\begin{theorem}[Complete word-observer blindness below (2m)]
If
...
then every coefficient-free word observer in (k) variables has the same count on
...
We now construct a distinguishing word observer in exactly (2m) variables. Let
...
\begin{proof}
The CSI statement is Theorem 8.1. The lower bound for word observers is Theorem 14.1. The upper bound is Theorem 15.1. The one-equation statement is Theorem 16.1.
...
pairwise nonisomorphic groups $G_f$ of order $p^{2m+2}$ in the same $\operatorname{CSI}_{\le m-1}$-fibre and in the same word-observer fibre below (2m) variables. In terms of
...
\section{Local observer lower bounds}
...
Then: $\operatorname{CSI}_{\le r}(\beta_{\mathcal F})$ determines and is determined by $D_{\mathcal F,\le r}$. Every coefficient-free word observer in (k) variables factors through $D_{\mathcal F,\le\lfloor k/2\rfloor}$. If $f,h$ are irreducible of degree (m), then all CSI observers below arity (m) and all word observers below (2m) variables are blind to the difference between $G_f$ and $G_h$. If $f,h$ are not projectively equivalent, then CSI at arity (m) and two explicit word equations in (2m) variables distinguish them. One word equation never distinguishes irreducible degree-(m) blocks. The associated directly indecomposable special groups have order $p^{2m+2}$, so the exact word-observer threshold is

\[
2m=\log_p|G_f|-2.
\]

A single lower-level observer fibre contains
...
\subsection{Logical observers}

The present paper proves lower bounds for coefficient-free quantifier-free word-count observers. Extending the factorization theorem to bounded-variable counting logic fragments with quantifier alternation would require a separate translation from formulas to spectral divisor data.
