...
In access-theoretic terms, a workload matrix is a finite linear observer system. Compressing that observer, or repairing a hidden fibre by adding local linear probes, is exactly the problem of factoring the corresponding workload through a small family of stored atoms. Thus sparse factorization is the linear algebraic mechanism behind local observer compression; it is not a metaphor for compression but the precise presentation model for exact linear access.
