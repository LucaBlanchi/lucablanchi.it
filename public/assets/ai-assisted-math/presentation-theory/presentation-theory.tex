...
\begin{abstract}
This note lays out a foundational formalism for presentation theory. It treats mathematical objects through chosen description systems, realization fibres, observables, indistinguishability fibres, resource scales, bounded parts, cost-controlled transfers, and simulations between presentation systems. The central objects are presentation systems and the filtrations, fibre profiles, observable profiles, reconstruction maps, normal-form resolutions, and observer-cost barriers they generate. The framework gives a common language for height theory, Kolmogorov complexity, circuit complexity, group presentations, algebraic straight-line complexity, finite test families, positive witness growth, and lower-bound criteria.
\end{abstract}
...
The first step is to choose what counts as the object class.
...
\[
C_\Gamma(x)=\inf\{\kappa(d): d\in\mathcal D,\ \rho(d)=x\}.
\]

In discrete settings this is often a minimum:

\[
C_\Gamma(x)=\min\{\kappa(d):\rho(d)=x\}.
\]
...
\[
\mathfrak F_\rho(x)=\rho^{-1}(x)=\{d\in\mathcal D:\rho(d)=x\}.
\]
...
Then every presentation fibre becomes a graph. Its vertices are the descriptions in \(\mathfrak F_\rho(x)\), and its edges are the elementary moves.
...
to a space of normal forms such that
...
The formal definition of a presentation system is intentionally broad.

A presentation system is paired with admissibility conditions appropriate to the application. Common conditions include:
\begin{enumerate}[label=(\roman*)]
\item the realization map is effectively computable, or at least mathematically canonical;
\item bounded description classes are finite, compact, constructible, recursively enumerable, or otherwise controllable;
\item the cost is monotone under the natural enlargements of descriptions;
\item equivalent descriptions of the same object have controlled effects on cost;
\item the chosen observables are canonical, or their selection cost is explicitly charged.
\end{enumerate}

These conditions keep the formalism aligned with the intended mathematical presentation problem.
...
x\notin\mathcal X_\Gamma^{\leq b}.
\]
If numerical presentation complexity is defined, then \(C_\Gamma(x)>b\).
\end{theorem}
...
\]
and an overhead function \(f\) such that
\[
...
\begin{corollary}[Effective move systems force nonrecursive bridge growth]
Let \(\mathcal M\) be an effective finite-branching move system on \(\mathcal D\) preserving realization. Fix \(d_x\in\mathfrak F_\rho(x)\). Suppose
\[
...
N(d)=N(d_x).
\]
Thus the fibre membership problem is decidable.
\end{proof}
...
Let \((X,\mu)\) be a finite measure space, and let \(\{A_c\}_{c\in\mathcal C}\) be a countable family of searched events. Suppose each event has a computable volume bound
\[
\mu(A_c)\leq V_c
\]
and a prefix-free search cost \(\ell(c)\) satisfying
\[
...
\[
\mu\left(
\bigcup_{\{c:\,V_c\leq \alpha 2^{-\ell(c)}\}} A_c
\right)
\leq \alpha.
\]
\end{theorem}
...
\right)
\leq
\sum_{\{c:\,V_c\leq \alpha 2^{-\ell(c)}\}} V_c
\leq
\alpha\sum_c2^{-\ell(c)}
\leq\alpha.
\]
...
This is the formal search-penalty principle behind adaptive discovery. The relevant budget is the description cost of the searched event.
