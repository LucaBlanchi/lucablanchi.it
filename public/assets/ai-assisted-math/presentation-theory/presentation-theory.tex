...
The word \(\pi\) is a membership certificate, or witness in the standard complexity-theoretic terminology, for the statement \(d\in\mathfrak F_\rho(x)\).

Define the minimum certificate length

\[
c_{R_x}(d)=\min\{|\pi|:R_x(d,\pi)\},
\]

for \(d\in\mathfrak F_\rho(x)\), and the bounded certificate-length profile
...
This profile measures the length of the shortest membership certificates needed to verify all fibre elements of coding length at most \(n\).
...
\section{Observer refinement, coarsening, and products}
...
\begin{theorem}[Unbounded certificate-length principle]
...
The theorem is elementary but important: recursively enumerable membership, once undecidable, forces shortest membership certificates to escape every computable uniform bound.

\begin{theorem}[Undecidable fibres force nonrecursive certificate-length growth]
...
\exists \pi\ R_x(d,\pi).
\]
Then the bounded certificate-length profile
\[
L_{R_x}(n)=
\max_{\substack{\ell(d)\leq n\\d\in\mathfrak F_\rho(x)}}
\min\{|\pi|:R_x(d,\pi)\}
...
\begin{proof}
Apply the unbounded certificate-length principle to the recursively enumerable undecidable set \(\mathfrak F_\rho(x)\), using the effective coding of descriptions.
\end{proof}
...
C_\Gamma(x)=0
\]
while the certificate-length profile of \(\mathfrak F_\rho(x)\) is not bounded above by any total computable function. Hence no function of minimal presentation complexity alone can bound fibre recognition or membership-verification cost in general.
\end{corollary}
...
u\in S,
\]
and let \(\mathcal X=\mathcal D/\!\equiv\), with \(\rho:\mathcal D\to\mathcal X\) the quotient map. Put \(\kappa(\ast)=0\) and \(\kappa(d_u)=1+|u|\). For \(x=\rho(\ast)\), one has \(C_\Gamma(x)=0\), while \(d_u\in\mathfrak F_\rho(x)\) exactly when \(u\in S\). The unbounded certificate-length principle gives the claimed nonrecursive growth.
\end{proof}
