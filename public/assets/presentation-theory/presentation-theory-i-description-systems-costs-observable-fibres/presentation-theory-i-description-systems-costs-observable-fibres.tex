...
\title{Presentation Theory I: Description Systems, Costs, and Observable Fibres}
\author{Luca Blanchi}
\date{June 2026}
...
\begin{abstract}
This note develops a formal language for studying mathematical objects through chosen systems of description. A presentation system consists of descriptions, a realization map, and resource bounds; it induces bounded parts, presentation complexity, and realization fibres. In the effective setting, a filtered presentation system has a decidable description language, effectively finite cost balls, and a filtered semidecision procedure for equivalence of descriptions. For such systems the bounded-presentation problem, the minimal-cost function, and the width profile for finding low-cost representatives have the same Turing degree:
\[
P_\Gamma\equiv_T C_\Gamma\equiv_T W_\Gamma.
\]
Similarly, equivalence of descriptions has the same Turing degree as the bridge-width profile:
\[
EQ_\Gamma\equiv_T B_\Gamma.
\]
Thus computable majorants for the relevant width profiles are exactly the effective content needed to decide bounded presentation or equivalence. The theory also treats observables and costed observables, whose fibres measure indistinguishability and whose bounded profiles give lower-bound criteria. Cost-controlled maps and normal-form compilers provide transfer principles between presentation systems. A central section collects reusable abstract theorems, including bounded-image obstructions, observable factorization, normal-form transfer, verification-length barriers, and no-resolution results, before the final examples specialize the language to heights, Kolmogorov complexity, circuits, group presentations, and algebraic complexity.
\end{abstract}
...
This gives three complementary layers: presentation complexity for objects, observable complexity for tests, and fibre structure for the information forgotten by a chosen family of observables. In many examples, all observables of bounded arity, degree, rank, or test size factor through a smaller structural statistic.
...
\section{Effectively finitary presentation systems}

In effective applications one often works with presentation systems whose bounded parts can actually be searched.

A presentation system
\[
\Gamma=(\mathcal D,\rho,\kappa)
\]
is \textbf{effectively finitary} if, for every cost bound \(b\),
\[
\mathcal D^{\leq b}:=\{d\in\mathcal D:\kappa(d)\leq b\}
\]
is finite and effectively enumerable, and the relevant realization comparisons
\[
\rho(d)\equiv_\mathcal X x
\]
are decidable for the bounded descriptions under consideration.

This condition is deliberately modest. It says that bounded search is meaningful once the bound is fixed. It does not say that the unbounded realization problem, equivalence problem, or fibre recognition problem is decidable.

\begin{proposition}[Bounded membership in effectively finitary systems]
Let \(\Gamma\) be effectively finitary. In any representation in which the comparisons \(\rho(d)\equiv_\mathcal X x\) are decidable for \(d\in\mathcal D^{\leq b}\), the predicate
\[
...
\begin{proof}
Enumerate the finite set \(\mathcal D^{\leq b}\) and test whether any enumerated description realizes an object equivalent to \(x\). The search terminates because the bounded description set is finite.
\end{proof}


\section{Filtered effective presentation systems}

The previous notion of effective finitarity says that bounded search is meaningful once a bound is fixed.  A sharper computability model records how equality of represented objects is witnessed by finite stages.

Fix a finite alphabet \(\Sigma\).  A \textbf{filtered effective presentation system} is a triple
\[
\Gamma=(D,\kappa,(E_t)_{t\in\mathbb N})
\]
such that:
\begin{enumerate}[label=(\roman*)]
\item \(D\subseteq\Sigma^*\) is decidable;
\item \(\kappa:D\to\mathbb N\) is computable and effectively proper, meaning that
\[
D_{\leq b}:=\{d\in D:\kappa(d)\leq b\}
\]
is finite and computable uniformly in \(b\);
\item each relation \(E_t\subseteq D\times D\) is decidable uniformly in \(t\);
\item \(E_t\subseteq E_{t+1}\) for all \(t\);
\item the union
\[
E=\bigcup_{t\in\mathbb N}E_t
\]
is an equivalence relation on \(D\).
\end{enumerate}
The represented object class is the quotient \(D/E\).  The realization map is the quotient map \(d\mapsto[d]\).

The \textbf{bounded-presentation problem} is
\[
P_\Gamma=\{(d,b)\in D\times\mathbb N:\exists e\in D_{\leq b}\text{ with }d\,E\,e\}.
\]
The minimal presentation cost of the object represented by \(d\) is
\[
C_\Gamma(d)=\min\{\kappa(e):e\,E\,d\}.
\]
For a positive instance \((d,b)\in P_\Gamma\), define its presentation width by
\[
w_\Gamma(d,b)=\min\{t:\exists e\in D_{\leq b}\text{ with }d\,E_t\,e\}.
\]
The global bounded-presentation width profile is
\[
W_\Gamma(N)=
\max\{w_\Gamma(d,b): |d|+b\leq N,\ (d,b)\in P_\Gamma\},
\]
with value \(0\) if the set is empty.

Similarly, define the equivalence problem
\[
EQ_\Gamma=\{(d,e)\in D\times D:d\,E\,e\}.
\]
For \(d\,E\,e\), define
\[
\beta_\Gamma(d,e)=\min\{t:d\,E_t\,e\},
\]
and define the bridge-width profile
\[
B_\Gamma(N)=
\max\{\beta_\Gamma(d,e): |d|+|e|\leq N,\ d\,E\,e\},
\]
again with value \(0\) if the set is empty.

\begin{theorem}[Degree conservation for bounded presentation]
For every filtered effective presentation system,
\[
P_\Gamma\equiv_T C_\Gamma\equiv_T W_\Gamma.
\]
\end{theorem}
...
\begin{proof}
The equivalence \(P_\Gamma\equiv_T C_\Gamma\) is immediate from the definition.  Given \(C_\Gamma\), decide \((d,b)\in P_\Gamma\) by testing \(C_\Gamma(d)\leq b\).  Given an oracle for \(P_\Gamma\), compute \(C_\Gamma(d)\) by testing \(b=0,1,\ldots,\kappa(d)\) and taking the least \(b\) with \((d,b)\in P_\Gamma\).

Now suppose \(W_\Gamma\) is available.  Given \((d,b)\), set \(N=|d|+b\) and \(M=W_\Gamma(N)\).  Since \(D_{\leq b}\) is finite and computable, one can decide whether
\[
\exists e\in D_{\leq b}\quad d\,E_M\,e.
\]
If such \(e\) exists, then \((d,b)\in P_\Gamma\).  If no such \(e\) exists and \((d,b)\) were positive, then by definition \(w_\Gamma(d,b)\leq W_\Gamma(N)=M\), contradiction.  Hence \(P_\Gamma\leq_T W_\Gamma\).

Conversely, assume an oracle for \(P_\Gamma\).  To compute \(W_\Gamma(N)\), list the finitely many pairs \((d,b)\) with \(d\in D\) and \(|d|+b\leq N\).  Use the oracle to keep exactly the positive pairs.  For each positive pair, search through \(t=0,1,2,\ldots\) until some \(e\in D_{\leq b}\) satisfies \(d\,E_t\,e\).  This search terminates by positivity.  Taking the maximum of the finitely many widths gives \(W_\Gamma(N)\).  Thus \(W_\Gamma\leq_T P_\Gamma\).
\end{proof}

\begin{theorem}[Degree conservation for equivalence]
For every filtered effective presentation system,
\[
EQ_\Gamma\equiv_T B_\Gamma.
\]
...
\begin{proof}
Given \(B_\Gamma\), decide whether \(d\,E\,e\) by setting \(N=|d|+|e|\), computing \(M=B_\Gamma(N)\), and testing the decidable relation \(d\,E_M\,e\).  If \(d\,E\,e\), then \(\beta_\Gamma(d,e)\leq M\), so the test succeeds; if the test succeeds, then \(d\,E\,e\) because \(E_M\subseteq E\).

Conversely, given an oracle for \(EQ_\Gamma\), list all pairs with \(|d|+|e|\leq N\), keep the equivalent pairs, compute each \(\beta_\Gamma(d,e)\) by finite-stage search, and take the maximum.  This computes \(B_\Gamma(N)\).
\end{proof}

\begin{corollary}[Computable majorants]
The following are equivalent:
\[
P_\Gamma\text{ is decidable},\qquad
C_\Gamma\text{ is computable},\qquad
W_\Gamma\text{ has a computable majorant}.
\]
Likewise,
\[
EQ_\Gamma\text{ is decidable}
\quad\Longleftrightarrow\quad
B_\Gamma\text{ has a computable majorant}.
\]
\end{corollary}

\begin{proof}
If the decision problem is decidable, the corresponding width profile is computable by the preceding proofs and hence majorizes itself.  Conversely, suppose \(g\) is a computable majorant for \(W_\Gamma\).  To decide \((d,b)\in P_\Gamma\), set \(N=|d|+b\) and test whether some \(e\in D_{\leq b}\) satisfies \(d\,E_{g(N)}\,e\).  The same argument used in the proof of degree conservation shows that this test is correct.  The bridge-width statement is identical, using \(g(|d|+|e|)\) in place of \(B_\Gamma(|d|+|e|)\).
\end{proof}

\begin{proposition}[Cofinal filtrations]
Let \((E_t)\) and \((F_s)\) be two decidable increasing filtrations of the same equivalence relation \(E\) on \(D\).  Suppose they are computably cofinal: there are computable functions \(\alpha,\beta:\mathbb N\to\mathbb N\) such that
\[
E_t\subseteq F_{\alpha(t)},\qquad F_s\subseteq E_{\beta(s)}.
\]
Then the corresponding \(W\)-profiles and \(B\)-profiles majorize each other up to computable change of variable.  In particular, the existence of a computable majorant is independent of replacing a filtration by a computably cofinal one.
\end{proposition}

\begin{proof}
If \(E_t\subseteq F_{\alpha(t)}\), let
\[
\alpha^*(m)=\max_{0\leq t\leq m}\alpha(t).
\]
Then every positive bounded-presentation instance witnessed by the \(E\)-filtration by stage \(m\) is witnessed by the \(F\)-filtration by stage \(\alpha^*(m)\).  Hence \(W_F(N)\leq\alpha^*(W_E(N))\).  The other inequality follows from \(\beta\).  The proof for bridge width is the same.
\end{proof}

\begin{proposition}[Sharpness of the two degrees]
Every c.e. Turing degree occurs as \(\deg_T(P_\Gamma)\) for some filtered effective presentation system.  More generally, for every pair of c.e. degrees \(\mathbf a\leq\mathbf b\), there is a filtered effective presentation system with
\[
\deg_T(P_\Gamma)=\mathbf a,
\qquad
\deg_T(EQ_\Gamma)=\mathbf b.
\]
\end{proposition}

\begin{proof}
For a c.e. set \(A=\bigcup_t A_t\), with \(A_t\) finite and decidable uniformly in \(t\), use descriptions \(0x\) and \(1x\).  Set
\[
\kappa(0x)=|x|,
\qquad
\kappa(1x)=|x|+1,
\]
and let \(0x\) and \(1x\) become equivalent at stage \(t\) precisely when \(x\in A_t\).  Then
\[
x\in A
\quad\Longleftrightarrow\quad
(1x,|x|)\in P_\Gamma,
\]
and the equivalence problem has the same degree.

For a pair \(\mathbf a\leq\mathbf b\), choose c.e. sets \(A,B\) with degrees \(\mathbf a,\mathbf b\) and \(A\leq_T B\).  Take the disjoint union of the preceding \(A\)-system with a second component whose nontrivial equivalences encode \(B\) but whose equivalent pairs have equal cost.  The second component contributes \(B\) to equivalence but only a decidable bounded-presentation problem.  The bounded-presentation degree is therefore \(\mathbf a\), while the equivalence degree is \(\mathbf b\).
\end{proof}
...
\section{Fibre membership and verification}
...
A \textbf{verification relation} for the fibre of \(x\) is a decidable relation
...
The datum \(\pi\) is membership verification data for the statement \(d\in\mathfrak F_\rho(x)\).

Define the minimum verification-data length

\[
c_{R_x}(d)=\min\{|\pi|:R_x(d,\pi)\},
\]

for \(d\in\mathfrak F_\rho(x)\), and the bounded verification-length profile
...
This profile measures the length of the shortest membership verification data needed to verify all fibre elements of coding length at most \(n\).
...
The overhead function \(f\) is part of the declared transfer data. It need not be optimal, but it must be monotone and compatible with the chosen source and target resource scales. In effective settings one usually requires \(f\) to be computable; in quantitative complexity settings one may restrict to linear, polynomial, or primitive recursive overheads.
...
It may also be called the \textbf{bounded observable image} of \(\mathcal X_\Gamma^{\leq b}\) under \(\omega\).
...
\section{Observable refinement, coarsening, and products}

Observables carry a simple algebra.
...
is a map, then
\[
q\circ\omega:\mathcal X\to B
\]
is a \textbf{coarsening} of \(\omega\).

An observable
\[
\eta:\mathcal X\to E
\]
\textbf{refines} \(\omega\) if \(\omega=q\circ\eta\) for some map \(q:E\to A\). Refinement can only distinguish at least as much as the original observable.
...
\eta:\mathcal X\to E,
\]
their product observable is
\[
(\omega,\eta):\mathcal X\to A\times E.
\]

For any presentation system \(\Gamma\) and cost bound \(b\),
\[
\operatorname{Val}_{\Gamma,(\omega,\eta)}(b)
\subseteq
\operatorname{Val}_{\Gamma,\omega}(b)\times
\operatorname{Val}_{\Gamma,\eta}(b).
\]

The inclusion can be strict. Strictness records correlations among observations imposed by the common bounded presentation class.
...
Examples of observation costs include arity of a word observable, length of a word, number or order of test groups, degree of a polynomial probe, rank or matrix size of a linear observable, number of evaluations, depth of a logical formula, or quantifier rank.

A declared costed observable family is also an \textbf{observable scheme}: it specifies which tests are allowed before a lower-bound argument begins. If an observable is selected using target-dependent information, then the selection procedure must be canonical or its cost must be included in \(\chi\). Otherwise singleton tests would trivialize distinguishability; such tests are part of the theory only when the declared observable scheme explicitly allows and charges for them.
...
Observable-barrier results have the form:
...
\[
\text{relative word observables}
\rightsquigarrow
\text{matrix-rank observables,}
\]
...
This principle reduces a universal statement to the bounded part \(\mathcal X_\Gamma^{\leq b}\): any failure of \(P\) has a low-cost counterexample.
...
\begin{proposition}[No universal bounded-counterexample procedure]
There is no algorithm which, given an arbitrary decidable predicate \(P(n)\) on \(\mathbb N\), outputs a finite set \(S\subseteq\mathbb N\) such that
\[
\exists n\ \neg P(n)
\quad\Longrightarrow\quad
\exists s\in S\ \neg P(s).
...
\begin{proof}
If such an algorithm existed, it would decide the halting problem. Given a Turing machine \(M\), let \(P_M(t)\) be the decidable predicate saying that \(M\) has not halted within \(t\) steps. Apply the hypothetical algorithm to \(P_M\), obtaining a finite set \(S\). Checking \(P_M(s)\) for all \(s\in S\) would decide whether some counterexample to \(P_M\) exists, equivalently whether \(M\) ever halts.
\end{proof}

Thus bounded-counterexample principles require separate mathematical input. They are powerful when available, but they cannot be generated uniformly from decidability alone.


\section{Reusable General Theorems}

The formalism gives stable proof patterns. The following theorems are abstract templates for height balls, coefficient boxes, word observables, hom-count tests, rank probes, normal forms, proof languages, and bounded counterexample searches.
...
\begin{theorem}[Observable factorization]
...
\]
such that every observable of cost at most \(r\) factors through \(\tau_r\) on \(\mathcal C\):
\[
...
\]
Thus observables of budget \(r\) distinguish objects only up to the fibres of \(\tau_r\).
\end{theorem}

\begin{proof}
If \(\tau_r(x)=\tau_r(y)\), then \(a_\omega(\tau_r(x))=a_\omega(\tau_r(y))\) for every factorizing observable \(\omega\).
\end{proof}

This theorem is the basic mechanism of observable complexity: bounded observables see the compressed statistic \(\tau_r\).

\begin{corollary}[Finite-observable blindness]
In the setting of the observable factorization theorem, if one fibre of \(\tau_r\) contains two non-equivalent objects \(x,y\in\mathcal C\), then \(\Omega^{\leq r}\) is not complete on \(\mathcal C\). In particular,
\[
...
This is the abstract form of many bounded-observable blindness results.

\begin{theorem}[Reconstruction from factored observables]
Keep the notation of the observable factorization theorem. Suppose, conversely, that \(\tau_r\) is determined by the observables \(\Omega^{\leq r}\): whenever \(x,y\in\mathcal C\) satisfy
\[
\omega(x)=\omega(y)
...
\begin{proof}
The observable factorization theorem gives one implication: equal \(\tau_r\) implies equal observations. The hypothesis gives the reverse implication. Hence the two equivalence relations coincide.
\end{proof}
...
\begin{theorem}[Unbounded verification-length principle]
...
The theorem is elementary but important: recursively enumerable membership, once undecidable, forces shortest membership verification data to escape every computable uniform bound.

\begin{theorem}[Undecidable fibres force nonrecursive verification-length growth]
...
\]
is recursively enumerable but not decidable. Suppose moreover that membership in this fibre is represented by a decidable verification relation
\[
\rho(d)=x
\quad\Longleftrightarrow\quad
\exists \pi\ R_x(d,\pi).
\]
Then the bounded verification-length profile
\[
L_{R_x}(n)=
\max_{\substack{\ell(d)\leq n\\d\in\mathfrak F_\rho(x)}}
\min\{|\pi|:R_x(d,\pi)\}
...
\begin{proof}
Apply the unbounded verification-length principle to the recursively enumerable undecidable set \(\mathfrak F_\rho(x)\), using the effective coding of descriptions.
\end{proof}

This turns undecidability of a presentation fibre into a quantitative lower bound on membership verification.
...
C_\Gamma(x)=0
\]
while the verification-length profile of \(\mathfrak F_\rho(x)\) is not bounded above by any total computable function. Hence no function of minimal presentation complexity alone can bound fibre recognition or membership-verification cost in general.
\end{corollary}

\begin{proof}
Let \(S\subseteq\{0,1\}^*\) be recursively enumerable but undecidable, with decidable verification relation \(R(u,\pi)\). Let
\[
...
u\in S,
\]
and let \(\mathcal X=\mathcal D/\!\equiv\), with \(\rho:\mathcal D\to\mathcal X\) the quotient map. Put \(\kappa(\ast)=0\) and \(\kappa(d_u)=1+|u|\). For \(x=\rho(\ast)\), one has \(C_\Gamma(x)=0\), while \(d_u\in\mathfrak F_\rho(x)\) exactly when \(u\in S\). The unbounded verification-length principle gives the claimed nonrecursive growth.
\end{proof}
...
The point of the corollary is that minimal presentation complexity alone cannot be a general bound for fibre recognition or membership-verification growth.
