...
\section{Minimal and Canonical Selectors}
...
\begin{definition}[Minimal selector]
...
\begin{definition}[Canonical minimal selector]
...
\begin{theorem}[Selector criterion]
For a filtered effective presentation system:
\begin{enumerate}[label=(\roman*)]
\item a computable minimal selector exists if and only if \(P_\Gamma\) is decidable;
\item a computable canonical minimal selector exists if and only if \(P_\Gamma\) and \(EQ_\Gamma\) are both decidable.
\end{enumerate}
\end{theorem}
...
\begin{remark}[Why this belongs to normal-form theory]
