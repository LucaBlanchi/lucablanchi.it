...
\begin{proposition}[Move filtrations and bridge width]
Assume the move relation \(M\) is decidable and that cost balls \(D^{\leq h}\) are finite and computable.  Define
\[
d\,E_h^M\,e
\]
to mean that \(d\) and \(e\) are connected in the finite \(M\)-graph induced on \(D^{\leq h}\).  Then \((E_h^M)\) is a decidable increasing filtration of the kernel pair \(K_\rho\).  Its bridge-width profile is Turing-equivalent to \(K_\rho\), and it has a computable majorant if and only if \(K_\rho\) is decidable.
\end{proposition}

\begin{proof}
Each \(E_h^M\) is decidable because the induced graph on the finite computable set \(D^{\leq h}\) is finite.  The relations are increasing in \(h\).  Soundness of moves gives \(E_h^M\subseteq K_\rho\), and completeness gives
\[
K_\rho=\bigcup_h E_h^M.
\]
Therefore this is a filtered effective presentation system for the kernel pair.  Applying the bridge-width theorem of Part I gives the stated Turing equivalence and the computable-majorant criterion.
\end{proof}

\begin{remark}
The navigation profile and the bridge-width profile are the same obstruction viewed with two different coordinates.  Navigation emphasizes geometry inside fibres; bridge width emphasizes the finite-stage semidecision procedure for equivalence.
\end{remark}
